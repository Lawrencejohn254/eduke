...
/**
 * Prints the same official receipt the bursar issues (same layout and wording), for a CONFIRMED payment
 * the parent can already see. Nothing new is fetched or written.
 */
export default function ReceiptButton({ data }: { data: ReceiptData }) {
  ...
  return (
    <button
      type="button"
      onClick={handlePrint}
      aria-label={`Print receipt ${data.receiptNumber ?? ""}`.trim()}
      className="inline-flex min-h-10 items-center gap-1.5 rounded-md px-2.5 text-[0.8125rem] font-medium text-pp-green hover:bg-pp-green-tint"
    >
      <Printer size={15} aria-hidden /> Receipt
    </button>
  );
}
