...
export default function SchemeRow({
  ...
  async function setStatus(newStatus: "Approved" | "Returned") {
    setBusy(true);
    ...
    await supabase
      ...
      .update({
        status: newStatus,
        hod_comments: newStatus === "Returned" ? comment : null,
  ...
  return (
    <>
      <tr className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer" onClick={() => setExpanded((v) => !v)}>
        ...
        <td className="p-3 text-gray-600">{klass}</td>
      ...
      {expanded && (
        <tr>
          <td colSpan={canReview ? 8 : 7} className="p-4 bg-gray-50">
            ...
            {hodComments && <p className="text-sm text-red-600 mt-2"><strong>HOD comments:</strong> {hodComments}</p>}
            {teacherResponse && <p className="text-sm text-blue-600 mt-1"><strong>Your response:</strong> {teacherResponse}</p>}
            {canReview && status === "Submitted" && (
              <div className="mt-3 space-y-2" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setStatus("Approved")} disabled={busy} className="flex items-center gap-1.5 bg-eduke-green text-white text-xs font-medium px-3 py-2 rounded-lg disabled:opacity-50">
                    <Check size={14} /> Approve
                  </button>
                  <button onClick={() => setStatus("Returned")} disabled={busy || !comment} className="flex items-center gap-1.5 bg-white border border-gray-300 text-xs font-medium px-3 py-2 rounded-lg disabled:opacity-50">
