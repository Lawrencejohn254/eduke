  ...
  async function handleSaveSlot(draft: SlotDraft) {
    ...
    if (draft.id) {
      ...
      if (!updated || updated.length === 0) {
        throw new Error("Nothing was saved — your session may have expired. Try logging out and back in, then retry.");
      }
    } else {
  ...
  async function handleLoadSample() {
    if (slots.length > 0) {
      const proceed = confirm(
        "You already have slots on your timetable. Loading the sample again will add duplicates in the same time slots. Continue anyway?"
      );
      if (!proceed) return;
    }
    setBusy(true);
                        ...
                        <td key={day} className="border border-gray-100 p-1.5 align-top">
                          {slot ? (
                            ...
                            >
                              ...
                              {slot.stream && (
                                <p className="text-[11px] font-semibold mt-1 flex items-center gap-1" style={{ color: c.text }}>
