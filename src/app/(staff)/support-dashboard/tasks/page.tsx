...
export default async function SupportTasksPage() {
  ...
  const { data: tasks, error } = staffRecord
    ? await supabase
        .from("staff_tasks")
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">Tasks assigned to you by your school administrators.</p>
      </div>
      ...
      <TasksClient tasks={tasks ?? []} unreadByTask={unreadByTask} />
    </div>
  );
}