...
import { Wallet, Award, CalendarCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Karibu, {profile.first_name}</h1>
          <p className="text-sm text-gray-500">{activeChild.first_name} {activeChild.last_name} · {activeChild.className} {activeChild.streamName}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link href="/parent/fees">
          <StatCard label="Fee Balance" value={formatKES(balance)} icon={Wallet} tone={balance > 0 ? "danger" : "default"} />
        </Link>
        <Link href="/parent/results">
          <StatCard
            label="Latest Result"
            value={lastResult ? `${(lastResult.subject as unknown as { name: string })?.name}: ${lastResult.grade}` : "No results yet"}
            icon={Award}
          />
        </Link>
        <Link href="/parent/attendance">
          <StatCard label="Attendance (30 days)" value={rate !== null ? `${rate}%` : "No data"} icon={CalendarCheck} tone={rate !== null && rate < 80 ? "danger" : "default"} />
        </Link>
      </div>
    </div>
  );
